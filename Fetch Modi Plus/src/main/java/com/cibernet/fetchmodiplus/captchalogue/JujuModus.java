...
import com.cibernet.fetchmodiplus.client.gui.captchalogue.JujuGuiHandler;
import com.cibernet.fetchmodiplus.network.FMPChannelHandler;
import com.cibernet.fetchmodiplus.network.FMPPacket;
import com.cibernet.fetchmodiplus.network.JujuModusPacket;
...
public class JujuModus extends BaseModus
{
	...
	public static void sendUpdateToClients(IdentifierHandler.PlayerIdentifier target, IdentifierHandler.PlayerIdentifier partner)
	{
		...
		if(!partnerNBT.hasNoTags() && target != null && target.getPlayer() != null && MinestuckPlayerData.getData(target).modus instanceof JujuModus)
		{
			...
			FMPPacket packet = FMPPacket.makePacket(FMPPacket.Type.UPDATE_MODUS, nbt);
			FMPChannelHandler.sendToPlayer(packet, target.getPlayer());
		}
		...
		if(!targetNBT.hasNoTags() && partner != null && partner.getPlayer() != null && MinestuckPlayerData.getData(partner).modus instanceof JujuModus)
		{
			...
			FMPPacket packet = FMPPacket.makePacket(FMPPacket.Type.UPDATE_MODUS, nbt);
			FMPChannelHandler.sendToPlayer(packet, partner.getPlayer());
		}
	}
	...
	public SylladexGuiHandler getGuiHandler()
	{
		...
		if(partnerID != -1 && displayStacks == null)
			FMPChannelHandler.sendToServer(FMPPacket.makePacket(FMPPacket.Type.JUJU_UPDATE, JujuModusPacket.Type.REQUEST_CLIENT));
	...
	public boolean canSwitchFrom(Modus modus)
	{
		...
		{
			if(side == Side.CLIENT)
				FMPChannelHandler.sendToServer(FMPPacket.makePacket(FMPPacket.Type.JUJU_UPDATE, JujuModusPacket.Type.UNLINK));
