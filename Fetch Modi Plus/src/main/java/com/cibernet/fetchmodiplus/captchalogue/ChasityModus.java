...
import com.cibernet.fetchmodiplus.client.gui.captchalogue.ChasityGuiHandler;
...
public class ChasityModus extends Modus
{
	...
	public ItemStack getItem(int id, boolean asCard)
	{
		...
		if(id == CaptchaDeckHandler.EMPTY_SYLLADEX)
		{
			...
			FMPChannelHandler.sendToPlayer(FMPPacket.makePacket(FMPPacket.Type.UPDATE_MODUS, CaptchaDeckHandler.writeToNBT(this)), player);
			return ItemStack.EMPTY;
		}
		...
		else if(list.get(id).locked && FetchModiPlus.isMSGTLoaded && MSGTPlayerData.clientData.isBadgeActive(MSGTBadges.SKELETON_KEY, player))
		{
			...
			player.world.playSound(null, player.getPosition(), FMPSounds.chasityUnlock, SoundCategory.PLAYERS, 0.8f, ((player.world.rand.nextFloat() - player.world.rand.nextFloat()) * 0.1F + 1.0F) * 0.95F);

		}
		else player.world.playSound(null, player.getPosition(), FMPSounds.chasityRattle, SoundCategory.PLAYERS, 0.8f, ((player.world.rand.nextFloat() - player.world.rand.nextFloat()) * 0.1F + 1.0F) * 0.95F);
	...
	{
		ItemStack key = new ItemStack(FMPItems.chasityKey, 1, MinestuckPlayerData.getData(player).color+1);
		...
		player.world.playSound(null, player.getPosition(), FMPSounds.chasityLock, SoundCategory.PLAYERS, 0.8f, ((player.world.rand.nextFloat() - player.world.rand.nextFloat()) * 0.1F + 1.0F) * 0.95F);
		...
		if(!livingList.isEmpty() && world.rand.nextInt(3) != 0)
		{
			...
			if(entity instanceof CruxiteSlimeEntity && ((CruxiteSlimeEntity) entity).getStoredItem().isEmpty())
			{
				((CruxiteSlimeEntity) entity).setStoredItem(key);
				return;
			}
