...
import com.cibernet.fetchmodiplus.client.gui.captchalogue.EnergyGuiHandler;
...
public class EnergyModus extends BaseModus
{
	...
	public boolean putItemStack(ItemStack stack)
	{
		...
		if(stack.getItem().equals(FMPItems.energyCell))
		{
			if(charge < getMaxCharge())
			{
				ItemStack spare = new ItemStack(FMPItems.energyCell, Math.max(0, stack.getCount() - (getMaxCharge()-charge)));
