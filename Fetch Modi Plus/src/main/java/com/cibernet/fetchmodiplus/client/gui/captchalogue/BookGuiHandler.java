...
public class BookGuiHandler extends BaseModusGuiHandler
{
	...
	protected void actionPerformed(GuiButton button) throws IOException
	{
		...
		if(button == guiButton)
		{
			FMPChannelHandler.sendToServer(FMPPacket.makePacket(FMPPacket.Type.BOOK_PUBLISH));
			modus.clear();
			updateContent();
		}
	}
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
	{
		...
		if(isMouseInContainer(mouseX, mouseY))
		{
			...
			{
				modus.page++;
				FMPChannelHandler.sendToServer(FMPPacket.makePacket(FMPPacket.Type.BOOK_UPDATE_PAGE, modus.page));
				updateContent();
