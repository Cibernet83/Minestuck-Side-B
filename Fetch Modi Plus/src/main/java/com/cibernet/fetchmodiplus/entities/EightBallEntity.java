...
public class EightBallEntity extends EntityThrowable
{
	protected ItemStack storedStack = ItemStack.EMPTY;
	
	public EightBallEntity(World worldIn)
	{
		super(worldIn);
	}
	
	public EightBallEntity(World worldIn, EntityLivingBase throwerIn, ItemStack stack)
	{
		super(worldIn, throwerIn);
		storedStack = stack;
	}
	
	public EightBallEntity(World worldIn, double x, double y, double z, ItemStack stack)
	{
		super(worldIn, x, y, z);
		storedStack = stack;
	}
	...
	public void handleStatusUpdate(byte id)
	{
		...
		{
			for (int i = 0; i < 8; ++i)
				this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, this.posX, this.posY, this.posZ, ((double)this.rand.nextFloat() - 0.5D) * 0.08D, ((double)this.rand.nextFloat() - 0.5D) * 0.08D, ((double)this.rand.nextFloat() - 0.5D) * 0.08D, Item.getIdFromItem(FMPItems.eightBall));
			for (int i = 0; i < 16; ++i)
