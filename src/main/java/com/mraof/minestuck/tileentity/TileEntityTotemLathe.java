...
public class TileEntityTotemLathe extends TileEntity
{
	...
	public void processContents()
	{
		...
		if(!dowel.isEmpty() && !AlchemyUtils.hasDecodedObject(dowel) &&  (!card1.isEmpty() || !card2.isEmpty()))
		{
			if(!card1.isEmpty() && !card2.isEmpty())
				...
				else output = CombinationRegistry.getCombination(AlchemyUtils.getDecodedItem(card1), AlchemyUtils.getDecodedItem(card2), CombinationRegistry.Mode.MODE_AND);
			else
			{
