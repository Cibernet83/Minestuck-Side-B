...
import com.mraof.minestuck.client.gui.captchalogue.sylladex.MultiSylladexGuiContainerLinear;
...
public abstract class Modus extends IForgeRegistryEntry.Impl<Modus> implements IRegistryObject<Modus>
{
	...
	/**
	 * Get whether the slots[i]th slot meets the modus's fetch requirements and satisfies its own requirements.
	 */
	public <SYLLADEX extends Sylladex> boolean canGet(SylladexList<SYLLADEX> sylladices, NBTTagCompound settings, int[] slots, int i)
	{
		return canGet(sylladices, settings, slots[i]) && sylladices.get(slots[i]).canGet(slots, i + 1);
	}

	/**
	 * Get whether the ith slot meets the modus's fetch requirements.
	 */
	 ...
	 */
	public <SYLLADEX extends Sylladex> void eject(SylladexList<SYLLADEX> sylladices, NBTTagCompound settings)
	{
	...
	{
		return new MultiSylladexGuiContainerLinear(sylladex, firstTextureIndices, lowerTextureIndices);
	}
