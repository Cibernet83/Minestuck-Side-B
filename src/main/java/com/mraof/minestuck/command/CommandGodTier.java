...
public class CommandGodTier extends CommandBase
{
	...
	{
		if (args.length < 1)
			throw new WrongUsageException("commands.godtier.usage");
		...
		switch (args[0])
		{
			...
			case "title":
				...
				notifyCommandListener(sender, this, 0, "commands.godtier.title.success", target.getDisplayName(), titleClass.asTextComponent(), titleAspect.asTextComponent());
			...
			case "reset":
				...
				notifyCommandListener(sender, this, 0, "commands.godtier.reset.success", target.getDisplayName());
				break;
			case "mastercontrol":

				if (args.length < 2 || (!args[1].equals("true") && !args[1].equals("false")))
					throw new WrongUsageException("commands.godtier.mastercontrol.usage");
				...
				notifyCommandListener(sender, this, 0, "commands.godtier.mastercontrol.success." + (enable ? "on" : "off"), target.getDisplayName());
				break;
			case "maxbadges":

				if (args.length < 2)
					throw new WrongUsageException("commands.godtier.maxbadges.usage");
				...
				try
				{
					...
					target.getCapability(MinestuckCapabilities.GOD_TIER_DATA, null).update();
					notifyCommandListener(sender, this, 0, "commands.godtier.maxbadges.success", target.getDisplayName(), count);
				}
				catch (NumberFormatException e)
				{
					throw new WrongUsageException("commands.generic.num.invalid", args[1]);
				}
			...
			case "lunarsway":
				...
				if (sway == null)
					throw new WrongUsageException("commands.godtier.lunarsway.usage");

				target = (args.length >= 3) ? getPlayer(server, sender, args[2]) : getCommandSenderAsPlayer(sender);
				target.getCapability(MinestuckCapabilities.GOD_TIER_DATA, null).setLunarSway(sway);
				target.getCapability(MinestuckCapabilities.GOD_TIER_DATA, null).update();
				notifyCommandListener(sender, this, 0, "commands.godtier.lunarsway." + args[1], target.getDisplayName());
				break;
			default:
				throw new WrongUsageException("commands.godtier.usage");
		}
