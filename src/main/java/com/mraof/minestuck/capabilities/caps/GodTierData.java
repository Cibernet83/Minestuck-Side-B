...
public class GodTierData implements IGodTierData
{
	...
	{
		return godTierXp.get(type).level * godTierXp.get(type).attributeMod * (isBadgeActive(MinestuckBadges.BADGE_PAGE) ? 2 : 1) * (isBadgeActive(MinestuckBadges.BADGE_OVERLORD) ? 3 : 1);
	}
