...
public class BookGuiHandler extends BaseModusGuiHandler
{
	...
	protected void actionPerformed(GuiButton button) throws IOException
	{
		...
		if(button == guiButton)
		{
			MinestuckChannelHandler.sendToServer(MinestuckPacket.makePacket(MinestuckPacket.Type.BOOK_PUBLISH));
			modus.clear();
			updateContent();
		}
	}
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
	{
		...
		if(isMouseInContainer(mouseX, mouseY))
		{
			...
			{
				modus.page++;
				MinestuckChannelHandler.sendToServer(MinestuckPacket.makePacket(MinestuckPacket.Type.BOOK_UPDATE_PAGE, modus.page));
				updateContent();
