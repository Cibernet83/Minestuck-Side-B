	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
	{
		...
		for(int i = 0; i < masterBadges.size(); i++)
		{
			...
			if(isPointInRegion(mouseX, mouseY, xOffset+(xSize - masterBadges.size()*22)/2  + i*22, yOffset+124, 20, 20) && badge.isReadable(player.world, player))
				MinestuckChannelHandler.sendToServer(MinestuckPacket.makePacket((!data.isBadgeActive(MinestuckBadges.BADGE_OVERLORD) && !data.hasBadge(badge) && data.getMasterBadge() == null && mouseButton == 0) ? MinestuckPacket.Type.ATTEMPT_BADGE_UNLOCK : MinestuckPacket.Type.TOGGLE_BADGE, badge));
		...
		for(int i = 0; i < badges.size(); i++)
		{
			...
			if(isPointInRegion(mouseX, mouseY, xOffset+(xSize - ((badges.size()+1)/rows)*22)/2  + ((i)/rows)*22, yOffset+157 + (i%rows)*22, 20, 20) && badge.isReadable(player.world, player))
				MinestuckChannelHandler.sendToServer(MinestuckPacket.makePacket((!data.hasBadge(badge) && mouseButton == 0) ? MinestuckPacket.Type.ATTEMPT_BADGE_UNLOCK : MinestuckPacket.Type.TOGGLE_BADGE, badge));
		}
	}
	...
	protected void upgradeSkills(int mouseX, int mouseY)
	{
		...
		for(int i = 1; i < GodTierData.SkillType.values().length; i++)
		{
			...
			if((player.isCreative() || player.experienceLevel >= SKILL_XP_THRESHOLD) && isPointInRegion(mouseX, mouseY, skillX+89, skillY, 18, 18))
			{
				...
				MinestuckChannelHandler.sendToServer(MinestuckPacket.makePacket(MinestuckPacket.Type.INCREASE_XP, skill, player, amount));
