...
public class BadgeEventHandler
{
	...
	public static void onLivingDamage(LivingDamageEvent event)
	{
		...
		if(!event.getSource().isUnblockable() && targetPlayer != null)
		{
			...
			{
				MinestuckChannelHandler.sendToTrackingAndSelf(MinestuckPacket.makePacket(MinestuckPacket.Type.SEND_PARTICLE, MinestuckParticles.ParticleType.AURA, 0x0094FF, 20, targetPlayer), targetPlayer);
				event.setAmount(0);
			}
		}
		...
		if(sourcePlayer != null && sourceData.isBadgeActive(MinestuckBadges.MASTER_BADGE_MIGHTY))
		{
			...
			{
				MinestuckChannelHandler.sendToTrackingAndSelf(MinestuckPacket.makePacket(MinestuckPacket.Type.SEND_PARTICLE, MinestuckParticles.ParticleType.AURA, 0xF80000, 20, sourcePlayer), sourcePlayer);
	...
	public static void onLivingDrops(LivingDropsEvent event)
	{
		if(event.getSource().getTrueSource() instanceof EntityPlayer)
		{
			...
			{
				MinestuckChannelHandler.sendToTracking(MinestuckPacket.makePacket(MinestuckPacket.Type.SEND_PARTICLE, MinestuckParticles.ParticleType.AURA, 0x00D54E, 20, entity.posX, entity.posY, entity.posZ), entity);
