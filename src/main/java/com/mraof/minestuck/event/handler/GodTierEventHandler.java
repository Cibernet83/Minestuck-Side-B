...
public class GodTierEventHandler
{
						...
						if(!gtData.isGodTier())
						{
							...
							for(EntityEquipmentSlot slot : EntityEquipmentSlot.values())
							{
								if(slot.getSlotType().equals(EntityEquipmentSlot.Type.ARMOR))
									SylladexUtils.launchItem(player, player.getItemStackFromSlot(slot));
							}
							...
							Item[] armor = new Item[]{MinestuckItems.gtShoes, MinestuckItems.gtPants, MinestuckItems.gtShirt, MinestuckItems.gtHood};
	...
	public static void playerDeathPost(LivingDeathEvent event)
	{
		if(event.isCanceled() && event.getEntityLiving().world.getWorldInfo().isHardcoreModeEnabled() && event.getEntityLiving() instanceof EntityPlayer && ((EntityPlayer) event.getEntityLiving()).isSpectator()
				&& event.getEntityLiving().hasCapability(MinestuckCapabilities.GOD_TIER_DATA, null) &&
				event.getEntityLiving().getCapability(MinestuckCapabilities.GOD_TIER_DATA, null).isGodTier())
	...
	public static void onClientTick(TickEvent.ClientTickEvent event)
	{
		...
		boolean canFly = !Minecraft.getMinecraft().player.isPotionActive(MinestuckPotions.EARTHBOUND);
	...
	public static final List<Potion> REFRESH_POTIONS = Arrays.asList(MobEffects.ABSORPTION, MobEffects.REGENERATION, MobEffects.WITHER, MobEffects.POISON, MinestuckPotions.GOD_TIER_COMEBACK);
	...
	public static void onTick(TickEvent.PlayerTickEvent event)
	{
		...
		if(gtData.isGodTier())
		{
			boolean canFly = !event.player.isPotionActive(MinestuckPotions.EARTHBOUND);
		...
		if(worlcC != null)
		{
			...
			if(!player.isCreative())
			{
						...
						&& player.posZ > questBedPos.getZ()-radius && player.posZ < questBedPos.getZ()+radius)
				{
					if(!gtData.isGodTier())
					{
						if(!gtData.climbedTheSpire() && (!player.isPotionActive(MinestuckPotions.EARTHBOUND) || player.getActivePotionEffect(MinestuckPotions.EARTHBOUND).getDuration() < 20))
							player.addPotionEffect(new PotionEffect(MinestuckPotions.EARTHBOUND, 40, 0, true, true));
						if(!player.isPotionActive(MinestuckPotions.CREATIVE_SHOCK) || player.getActivePotionEffect(MinestuckPotions.CREATIVE_SHOCK).getDuration() < 20)
							player.addPotionEffect(new PotionEffect(MinestuckPotions.CREATIVE_SHOCK, 40, 0, true, true));
					}
				}
		...
		if(MinestuckConfig.aspectEffects && c != null && c.enteredGame())
		{
			...
			if (MinestuckPlayerData.getEffectToggle(identifier) && !player.isPotionActive(MinestuckPotions.GOD_TIER_LOCK))
			...
			else if(gtData.isGodTier() || player.isPotionActive(MinestuckPotions.GOD_TIER_LOCK))
	...
	public static HashMap<Potion, PotionEffect> getAspectEffects(EntityPlayer player)
	{
		...
		int potionLevel = (int)(aspectStrength[aspect.ordinal()] * (float)(gtData.isGodTier() ? 60 : rung)) + (gtData.isBadgeActive(MinestuckBadges.BADGE_PAGE) ? 2 : 0);
		HashMap<Potion, PotionEffect> appliedPotions = new HashMap<>();
		if (gtData.isBadgeActive(MinestuckBadges.EFFECT_BUFF))
			switch (aspect)
			{
				case DOOM:
					...
					break;
				case HOPE:
					appliedPotions.put(MinestuckPotions.DECAYPROOF, new PotionEffect(MinestuckPotions.DECAYPROOF, 600, 0, true, false));
					break;
				case MIND:
					if (!(player.getActivePotionEffect(MobEffects.GLOWING) != null && player.getActivePotionEffect(MobEffects.GLOWING).getAmplifier() >= 2))
						appliedPotions.put(MinestuckPotions.MIND_FORTITUDE, new PotionEffect(MinestuckPotions.MIND_FORTITUDE, 600, 0, true, false));
					break;
				case VOID:
					appliedPotions.put(MinestuckPotions.VOID_CONCEAL, new PotionEffect(MinestuckPotions.VOID_CONCEAL, 600, 0, true, false));
					break;
				default:
					potionLevel *= 2;
					break;
	...
	public static void onPlayerHurt(LivingHurtEvent event)
	{
		...
		IGodTierData data = event.getEntityLiving().getCapability(MinestuckCapabilities.GOD_TIER_DATA, null);
		float dmgReduction = data.getSkillLevel(GodTierData.SkillType.DEFENSE) * 0.002f * (data.isBadgeActive(MinestuckBadges.BADGE_PAGE) ? 2 : 1);
	...
	public static void onPlayerJoinWorld(EntityJoinWorldEvent event)
	{
		if(event.getEntity() instanceof EntityPlayerMP)
		{
			...
			IGodTierData data = event.getEntity().getCapability(MinestuckCapabilities.GOD_TIER_DATA, null);
			...
			MinestuckChannelHandler.sendToPlayer(MinestuckPacket.makePacket(MinestuckPacket.Type.UPDATE_DATA_SERVER, ((EntityPlayer)event.getEntity())), ((EntityPlayer)event.getEntity()));
		}
	}
}
