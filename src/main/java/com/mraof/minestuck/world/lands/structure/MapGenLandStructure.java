...
public class MapGenLandStructure extends MapGenStructure
{
	...
	protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ)    //This works very much like the scattered features in the overworld
	{
		...
		if (x < 0)
			x -= MAX_STRUCTURE_DISTANCE - 1;
		if (z < 0)
			z -= MAX_STRUCTURE_DISTANCE - 1;

		x /= MAX_STRUCTURE_DISTANCE;
		z /= MAX_STRUCTURE_DISTANCE;
		Random random = this.world.setRandomSeed(x, z, 59273643 ^ world.provider.getDimension());
		x *= MAX_STRUCTURE_DISTANCE;
		z *= MAX_STRUCTURE_DISTANCE;
		x += random.nextInt(MAX_STRUCTURE_DISTANCE - MIN_STRUCTURE_DISTANCE);
		z += random.nextInt(MAX_STRUCTURE_DISTANCE - MIN_STRUCTURE_DISTANCE);
	...
	public void placeReturnNodes(World world, Random rand, ChunkPos coords, BlockPos decoratorPos)
	{
		...
		if (x < 0)
			x -= MAX_NODE_DISTANCE - 1;
		if (z < 0)
			z -= MAX_NODE_DISTANCE - 1;
		...
		x *= MAX_NODE_DISTANCE;
		z *= MAX_NODE_DISTANCE;
		x += random.nextInt(MAX_NODE_DISTANCE - MIN_NODE_DISTANCE);
		z += random.nextInt(MAX_NODE_DISTANCE - MIN_NODE_DISTANCE);
