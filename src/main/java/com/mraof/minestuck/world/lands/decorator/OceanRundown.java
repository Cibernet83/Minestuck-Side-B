	...
	public BlockPos generate(World world, Random random, int chunkX, int chunkZ, ChunkProviderLands provider)
	{
		...
		if (random.nextFloat() < probability)
		{
			...
			for (int i = 0; i < attempts; i++)
			{
				...
				check:
				for (int posX = pos1.getX() - 3; posX <= pos1.getX() + 3; posX++)    //Check if the position generated is on land a distance away from ocean
				{
					for (int posZ = pos1.getZ() - 3; posZ <= pos1.getZ() + 3; posZ++)
					{
						if (!biomes[posX + 32 * posZ].equals(MinestuckBiomes.mediumNormal)
									&& !biomes[posX + 32 * posZ].equals(MinestuckBiomes.mediumRough))
						{
							condition = false;
							break check;
						}
				...
				if (condition) //Look for ocean and pick pos2 and pos3
				{
					...
					for (int posX = pos1.getX() - 8; posX < pos1.getX() + 8; posX++)
					{
						...
						{
							if (biomes[posX + posZ * 32].equals(MinestuckBiomes.mediumOcean))
