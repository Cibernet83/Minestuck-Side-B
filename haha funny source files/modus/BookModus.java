...
import com.mraof.minestuck.client.gui.captchalogue.BookGuiHandler;
...
public class BookModus extends Modus
{
	public int page = 0;
	...
	public ItemStack createBook()
	{
		...
		ItemStack book = new ItemStack(MinestuckItems.captchalogueBook, 1, MinestuckPlayerData.getData(player).color + 1);
	...
	{
		if(stack.getItem().equals(MinestuckItems.captchalogueBook))
		{
			...
			if(result)
			{
				if(!player.addItemStackToInventory(currBook))
					SylladexUtils.launchItem(player, currBook);
			}
			return result;
		}
	...
	public ItemStack getItem(int id, boolean asCard)
	{
		...
		if(id == SylladexUtils.EMPTY_CARD || asCard)
