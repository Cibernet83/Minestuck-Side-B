...
import com.mraof.minestuck.client.gui.captchalogue.ChasityGuiHandler;
...
public class ChasityModus extends Modus
{
	...
	public ItemStack getItem(int id, boolean asCard)
	{
		...
		if(id == SylladexUtils.EMPTY_SYLLADEX)
		{
			for(ChasityEntry entry : list)
			{
				...
				if(canUse(i))
				{
					SylladexUtils.launchItem(player, entry.stack);
					list.remove(i);
				}
			}
		...
		/* else if(list.get(id).locked && Minestuck.isMSGTLoaded && MSGTPlayerData.clientData.isBadgeActive(MSGTBadges.SKELETON_KEY, player)) // FIXME: re enable this when msgt lol
		{
			list.get(id).locked = false;
			if(side.isServer())
			{
				player.sendStatusMessage(new TextComponentTranslation("status.chasityUnlock"), true);
				MinestuckChannelHandler.sendToPlayer(MinestuckPacket.makePacket(MinestuckPacket.Type.UPDATE_MODUS, CaptchaDeckHandler.writeToNBT(this)), player);
			}
			player.world.playSound(null, player.getPosition(), MinestuckSounds.chasityUnlock, SoundCategory.PLAYERS, 0.8f, ((player.world.rand.nextFloat() - player.world.rand.nextFloat()) * 0.1F + 1.0F) * 0.95F);

		}*/
		else player.world.playSound(null, player.getPosition(), MinestuckSounds.chasityRattle, SoundCategory.PLAYERS, 0.8f, ((player.world.rand.nextFloat() - player.world.rand.nextFloat()) * 0.1F + 1.0F) * 0.95F);
	...
	{
		ItemStack key = new ItemStack(MinestuckItems.chasityKey, 1, MinestuckPlayerData.getData(player).color + 1);
		...
		player.world.playSound(null, player.getPosition(), MinestuckSounds.chasityLock, SoundCategory.PLAYERS, 0.8f, ((player.world.rand.nextFloat() - player.world.rand.nextFloat()) * 0.1F + 1.0F) * 0.95F);
