...
import com.mraof.minestuck.client.gui.captchalogue.CommunistGuiHandler;
...
public class CommunistModus extends Modus
{
	...
	public void initModus(NonNullList<ItemStack> prev, int prevSize)
	{
		...
		if(MinestuckConfig.modusMaxSize > 0)
			for(int i = 0; i < remainingCards; i++)
				SylladexUtils.launchItem(player, new ItemStack(MinestuckItems.captchaCard, 1));
	...
	public void readFromNBT(NBTTagCompound nbtTagCompound)
	{
		...
		if (this.side.isClient()) {
			...
			this.changed = true;
			MinestuckChannelHandler.sendToServer(MinestuckPacket.makePacket(MinestuckPacket.Type.REQUEST_COM_UPDATE));
	...
	public boolean sendUpdate()
	{
		if(!player.world.isRemote)
			for(EntityPlayer p : player.world.getPlayers(EntityPlayer.class, (e) -> MinestuckPlayerData.getData(e).modus instanceof CommunistModus))
				MinestuckChannelHandler.sendToPlayer(MinestuckPacket.makePacket(MinestuckPacket.Type.COM_UPDATE, writeToNBTGlobal(new NBTTagCompound())), p);
		return true;
	}
	...
	{
		if(!player.world.isRemote)
			MinestuckChannelHandler.sendToPlayer(MinestuckPacket.makePacket(MinestuckPacket.Type.COM_UPDATE, writeToNBTGlobal(new NBTTagCompound())), player);
		return true;
	}
