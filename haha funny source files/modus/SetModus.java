...
import com.mraof.minestuck.client.gui.captchalogue.SetGuiHandler;
...
public class SetModus extends Modus
{
	...
	public boolean putItemStack(ItemStack item)
	{
		...
		for(ItemStack stack : list)
			...
			{
				SylladexUtils.insertCardsOrLaunchItem(player, item);
				return true;
		...
		if(item.getCount() > 1)
		{
			...
			stack.shrink(1);
			item.setCount(1);
			SylladexUtils.insertCardsOrLaunchItem(player, stack);
		}
		list.add(item);
