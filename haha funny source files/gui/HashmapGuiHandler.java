...
import com.mraof.minestuck.inventory.captchalouge.HashtableModus;
...
public class HashmapGuiHandler extends SylladexGuiHandler
{
	...
	private HashtableModus modus;
	protected GuiButton guiButton;
	
	public HashmapGuiHandler(HashtableModus modus)
	{
		super();
		this.modus = modus;
		this.textureIndex = 4;
		...
		for(int i = 0; i < stacks.size(); i++)
			this.cards.add(new GuiCard(stacks.get(i), this, i, start + i*(CARD_WIDTH + 5), (mapHeight - CARD_HEIGHT)/2)
			{
				...
				public void onClick(int mouseButton)
				{
					...
					{
						MinestuckPacket packet = MinestuckPacket.makePacket(Type.CAPTCHA, PacketCaptchaDeck.GET, this.index, true);
	...
	protected void actionPerformed(GuiButton button) throws IOException
	{
		...
		if(button == this.guiButton && MinestuckConfig.clientHashmapChat == 0)
		{
			...
			MinestuckChannelHandler.sendToServer(MinestuckPacket.makePacket(MinestuckPacket.Type.CAPTCHA, PacketCaptchaDeck.VALUE, (byte) 0, modus.ejectByChat ? 1 : 0));
		}
	}
}