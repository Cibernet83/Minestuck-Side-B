...
import com.cibernet.minestuckgodtier.badges.heroAspect.BadgeHeroAspect;
import com.cibernet.minestuckgodtier.badges.heroAspectUtil.BadgeHeroAspectUtil;
import com.cibernet.minestuckgodtier.badges.heroClass.BadgeHeroClass;
...
import com.cibernet.minestuckgodtier.util.EnumLunarSway;
...
public class GodTierData implements IGodTierData
{
	...
	{
		if(badge instanceof MasterBadge)
			return badge == masterBadge || (isBadgeEnabled(MSGTBadges.BADGE_OVERLORD));
	...
	{
		if(badge instanceof MasterBadge && isBadgeActive(MSGTBadges.BADGE_OVERLORD))
	...
	public void increaseXp(SkillType type, float value)
	{
		...
		if(!hasBadge(MSGTBadges.GIFT_OF_GAB) && getSkillLevel(SkillType.GENERAL) > 1)
			this.badges.put(MSGTBadges.GIFT_OF_GAB, true);
	...
	{
		return godTierXp.get(type).level * godTierXp.get(type).attributeMod * (isBadgeActive(MSGTBadges.BADGE_PAGE) ? 2 : 1) * (isBadgeActive(MSGTBadges.BADGE_OVERLORD) ? 3 : 1);
	}
	...
	{
		if(owner.isUser())
			MSGTChannelHandler.sendToServer(MSGTPacket.makePacket(MSGTPacket.Type.UPDATE_DATA_CLIENT));
		else
			MSGTChannelHandler.sendToPlayer(MSGTPacket.makePacket(MSGTPacket.Type.UPDATE_DATA_SERVER, owner), owner);
	}
	...
	public void readFromNBT(NBTTagCompound nbt)
	{
		...
		for(int i = 0; i < maxBadges && badges.hasKey(String.valueOf(i)); i++)
		{
			...
			Badge badge = MSGTBadges.REGISTRY.getValue(new ResourceLocation(badgeData.getString("ID")));
			if(badge != null)
				this.badges.put(badge, badgeData.getBoolean("Enabled") || !badge.canDisable());
		}
		if(nbt.hasKey("MasterBadge"))
		{
			Badge badge = MSGTBadges.REGISTRY.getValue(new ResourceLocation(nbt.getString("MasterBadge")));
			if(badge instanceof MasterBadge)
				masterBadge = (MasterBadge) badge;
		}
	...
	{
		event.getEntity().getCapability(MSGTCapabilities.GOD_TIER_DATA, null).readFromNBT(event.getOriginal().getCapability(MSGTCapabilities.GOD_TIER_DATA, null).writeToNBT());
	}
}