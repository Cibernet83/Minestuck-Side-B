...
import com.cibernet.minestuckgodtier.entities.ai.EntityAIFollowReformer;
import com.cibernet.minestuckgodtier.entities.ai.EntityAINearestNonReformer;
...
public class BadgeUtilBlood extends BadgeHeroAspectUtil
{
	...
	public void onBadgeUnlocked(World world, EntityPlayer player) {
		...
		player.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).setReforming(true);
	}
	...
	{
		if(event.getEntityPlayer().getCapability(MSGTCapabilities.GOD_TIER_DATA, null) != null &&
			event.getEntityPlayer().getCapability(MSGTCapabilities.GOD_TIER_DATA, null).isBadgeActive(MSGTBadges.BADGE_UTIL_BLOOD))
