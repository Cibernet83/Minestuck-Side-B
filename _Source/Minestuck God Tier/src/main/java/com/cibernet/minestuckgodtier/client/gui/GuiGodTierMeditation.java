...
{
	public static final ResourceLocation TEXTURES = new ResourceLocation(MinestuckGodTier.MODID, "textures/gui/god_tier_meditation.png");
	public static final ResourceLocation MOCKUP = new ResourceLocation(MinestuckGodTier.MODID, "textures/gui/god_tier_meditation_mockup.png");
	...
	protected void setupBadges()
	{
		...
		for(Map.Entry<ResourceLocation, Badge> entry : MSGTBadges.REGISTRY.getEntries())
		{
		...
		if(data.hasMasterControl() && badges.contains(MSGTBadges.BADGE_OVERLORD))
		{
			badges.remove(MSGTBadges.BADGE_OVERLORD);
			badges.add(badges.indexOf(MSGTBadges.BADGE_LORD)+1, MSGTBadges.BADGE_OVERLORD);
		}
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks)
	{
		...
		boolean isOverlord = data.isBadgeActive(MSGTBadges.BADGE_OVERLORD);
		...
		for(int i = 1; i < GodTierData.SkillType.values().length; i++)
		{
			...
			drawTexturedModalRect(skillX + 89, skillY, ((player.isCreative() || player.experienceLevel >= MinestuckGodTier.SKILL_XP_THRESHOLD) ? isPointInRegion(mouseX, mouseY, skillX+89, skillY, 18, 18) ? (mouseClicked ? 36 : 18) : 0 : 36), 20, 18, 18);
			renderBorderedText(skillX+9 - (fontRenderer.getStringWidth(String.valueOf(level))/2), skillY+9 - (fontRenderer.FONT_HEIGHT/2), String.valueOf(level),  data.isBadgeActive(MSGTBadges.BADGE_PAGE) ? 0xFFD84C : (isOverlord ? 0xFF0000 : 0x80FF20), 0);
		}
		...
		for(int i = 1; i < GodTierData.SkillType.values().length; i++)
		{
			...
			if(isPointInRegion(mouseX, mouseY, skillX, skillY, 18, 18))
			{
				...
				if(skill.equals(GodTierData.SkillType.DEFENSE))
					tooltip.add(I18n.format("godTierSkill.defense.tooltipDesc2", ItemStack.DECIMALFORMAT.format(level*0.2 * (data.isBadgeActive(MSGTBadges.BADGE_PAGE) ? 2 : 1) * (data.isBadgeActive(MSGTBadges.BADGE_OVERLORD) ? 3 : 1))));
			...
			else if((!player.isCreative() && player.experienceLevel < MinestuckGodTier.SKILL_XP_THRESHOLD) && isPointInRegion(mouseX, mouseY, skillX+89, skillY, 18, 18))
				drawHoveringText(I18n.format("gui.needXp", MinestuckGodTier.SKILL_XP_THRESHOLD), mouseX, mouseY);
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
	{
		...
		for(int i = 0; i < masterBadges.size(); i++)
		{
			...
			if(isPointInRegion(mouseX, mouseY, xOffset+(xSize - masterBadges.size()*22)/2  + i*22, yOffset+124, 20, 20) && badge.isReadable(player.world, player))
				MSGTChannelHandler.sendToServer(MSGTPacket.makePacket((!data.isBadgeActive(MSGTBadges.BADGE_OVERLORD) && !data.hasBadge(badge) && data.getMasterBadge() == null && mouseButton == 0) ? MSGTPacket.Type.ATTEMPT_BADGE_UNLOCK : MSGTPacket.Type.TOGGLE_BADGE, badge));
		...
		for(int i = 0; i < badges.size(); i++)
		{
			...
			if(isPointInRegion(mouseX, mouseY, xOffset+(xSize - ((badges.size()+1)/rows)*22)/2  + ((i)/rows)*22, yOffset+157 + (i%rows)*22, 20, 20) && badge.isReadable(player.world, player))
				MSGTChannelHandler.sendToServer(MSGTPacket.makePacket((!data.hasBadge(badge) && mouseButton == 0) ? MSGTPacket.Type.ATTEMPT_BADGE_UNLOCK : MSGTPacket.Type.TOGGLE_BADGE, badge));
		}
	}
	...
	protected void upgradeSkills(int mouseX, int mouseY)
	{
		...
		for(int i = 1; i < GodTierData.SkillType.values().length; i++)
		{
			...
			if((player.isCreative() || player.experienceLevel >= MinestuckGodTier.SKILL_XP_THRESHOLD) && isPointInRegion(mouseX, mouseY, skillX+89, skillY, 18, 18))
			{
				mc.getSoundHandler().playSound(PositionedSoundRecord.getMasterRecord(SoundEvents.UI_BUTTON_CLICK, 1.0F));

				int amount = showExtra ? 5 : 1;
				MSGTChannelHandler.sendToServer(MSGTPacket.makePacket(MSGTPacket.Type.INCREASE_XP, skill, player, amount));
