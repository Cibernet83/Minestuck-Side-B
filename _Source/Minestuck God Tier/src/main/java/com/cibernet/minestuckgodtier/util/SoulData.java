...
public class SoulData
{
	...
	public SoulData(EntityLivingBase player)
	{
		...
		this.decayTime = player.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).getDecayTime();
	...
	public void apply(EntityLivingBase player)
	{
		...
		player.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).setDecayTime(decayTime);

		if(player instanceof EntityPlayer)
		{
			((EntityPlayer) player).inventory.currentItem = this.currentItem;
			if(FMLCommonHandler.instance().getSide() == Side.SERVER)
				MSGTChannelHandler.sendToPlayer(MSGTPacket.makePacket(MSGTPacket.Type.SET_CURRENT_ITEM, this.currentItem), (EntityPlayer) player);
		}
	...
	{
		Vec3d prevPos = entity.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).getPrevPos();
