...
public class ItemLightSword extends ItemPotionWeapon
{
    ...
    public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase player)
    {
        if(target.getCreatureAttribute() == EnumCreatureAttribute.UNDEAD)
            target.attackEntityFrom(player instanceof EntityPlayer ? DamageSource.causePlayerDamage((EntityPlayer) player) : DamageSource.causeMobDamage(player), getAttackDamage());
