...
public class BlockHeroStone extends Block implements IGodTierBlock
{
	...
	{
		super(Material.ROCK, getAspectMapColor(aspect));
		
		this.aspect = aspect;
		this.isChiseled = isChiseled;
		
		setUnlocalizedName("heroStone" + (isChiseled ? "Chiseled" : ""));
		setRegistryName((isChiseled ? "chiseled_" : "") + "hero_stone" + (aspect == null ? "" : ("_" + aspect.toString())));
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
	{
		...
		if(stack.getItem().equals(MinestuckItems.moonstoneChisel))
		{
			if(!worldIn.isRemote)
			{
				...
				InventoryHelper.spawnItemStack(worldIn,pos.getX(),pos.getY(),pos.getZ(), new ItemStack(MSGTItems.heroStoneShards.get(aspect), val));
		...
		else if(!(playerIn instanceof FakePlayer))
		{
			...
			if(!playerIn.isSneaking() && playerIn.getCapability(MSGTCapabilities.GOD_TIER_DATA, null).isGodTier() && (aspect == null || aspect == title.getHeroAspect()) && facing == EnumFacing.UP)
			{
				if(worldIn.isRemote)
					playerIn.openGui(MinestuckGodTier.instance, MSGTGuiHandler.MEDITATE, worldIn, pos.getX(), pos.getY(), pos.getZ());
				return true;
			}
		}
