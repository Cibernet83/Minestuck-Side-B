...
public class BadgeHeir extends BadgeHeroClass
{
	...
	{
		if(target != null && target.getCapability(MSGTCapabilities.GOD_TIER_DATA, null).isBadgeActive(MSGTBadges.BADGE_HEIR))
		{
			if(amount == -1 || (target.world.rand.nextFloat() < Math.max(0.1f, amount/target.getHealth()*0.8f * (target.getLuck()/4f-0.2f))))
			{
				...
				if(title != null)
				{
					...
					trueSource.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).oneshotPowerParticles(MSGTParticles.ParticleType.AURA, EnumClass.HEIR, 5);
				}
			}
		}
	}
