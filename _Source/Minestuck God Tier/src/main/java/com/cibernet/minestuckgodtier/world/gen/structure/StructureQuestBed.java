package com.cibernet.minestuckgodtier.world.gen.structure;
...
public class StructureQuestBed extends WorldGenerator
{
	...
	public static final int MIN_RADIUS = 6000;
	...
	public boolean generate(World worldIn, Random rand, BlockPos position)
	{
		...
		if(worldIn.provider.createChunkGenerator() instanceof ChunkProviderLands && position.getX()/16 == chunk.x && position.getZ()/16 == chunk.z)
		{
			...
			IBlockState wall = (aspect == null ? MSGTBlocks.wildcardHeroStoneWall : MSGTBlocks.heroStoneWalls.get(aspect)).getDefaultState();
			IBlockState light = MSGTBlocks.glowingHeroStone.getDefaultState();
			...
			for(int xOff = -1; xOff <= 1; xOff++)
				for(int zOff = -1; zOff <= 2; zOff++)
					setBlockAndNotifyAdequately(worldIn, new BlockPos(position.getX() + xOff, top, position.getZ() + zOff), (
							xOff == 0 && zOff == 0 ?
									(aspect == null ? MSGTBlocks.wildcardChiseledHeroStone : MSGTBlocks.chiseledHeroStones.get(aspect)) :
									(aspect == null ? MSGTBlocks.wildcardHeroStone : MSGTBlocks.heroStones.get(aspect))
							).getDefaultState());
