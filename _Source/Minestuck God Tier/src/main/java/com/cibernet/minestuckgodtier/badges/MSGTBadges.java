...
import com.cibernet.minestuckgodtier.badges.heroClass.*;
...
@Mod.EventBusSubscriber(modid = MinestuckGodTier.MODID)
public class MSGTBadges
{
	...
	public static final Badge SKELETON_KEY = new BadgeLevel("skeletonKey", 3)
	{
		...
		public boolean canUnlock(World world, EntityPlayer player)
		{
			...
			if(MinestuckGodTier.isFMPLoaded)
			...
			else canUnlock = canUnlock && GristHelper.canAfford(MinestuckPlayerData.getGristSet(player), cost);
		...
		public String getUnlockRequirements() {
			return MinestuckGodTier.isFMPLoaded ? new TextComponentTranslation(getUnlocalizedName()+".unlock.fmp").getFormattedText() : super.getUnlockRequirements();
		}
	};
	...
	public static final Badge HOARD_OF_THE_ALCHEMIZER = new BadgeLevel("hoardOfTheAlchemizer", 4)
	{
		...
		{
			MSGTChannelHandler.sendToPlayer(MSGTPacket.makePacket(MSGTPacket.Type.REQUEST_GRIST_HOARD), player);
		}
		...
		{
			return Minecraft.getMinecraft().player.getCapability(MSGTCapabilities.GOD_TIER_DATA, null).hasBadge(this);
		}
		@SideOnly(Side.CLIENT)
		private GristType getGristHoard()
		{
			return Minecraft.getMinecraft().player.getCapability(MSGTCapabilities.GOD_TIER_DATA, null).getGristHoard();
		}
	};
	...
	{
		REGISTRY = (ForgeRegistry)(new RegistryBuilder()).setName(new ResourceLocation(MinestuckGodTier.MODID, "god_tier_badges")).setDefaultKey(new ResourceLocation(MinestuckGodTier.MODID)).setType(Badge.class).create();
	}
}
