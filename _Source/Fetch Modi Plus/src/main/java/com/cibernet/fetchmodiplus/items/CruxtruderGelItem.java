	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
	{
		...
		if(worldIn.getTileEntity(pos) instanceof TileEntitySburbMachine)
		{
			...
			if(te.getMachineType() == BlockSburbMachine.MachineType.CRUXTRUDER)
			{
				...
				worldIn.playSound(null, pos, FMPSounds.cruxtruderGelFill, SoundCategory.BLOCKS, 0.8f, 1);
				if(worldIn.isRemote) for (int i = 0; i < 16; ++i)
					FMPParticles.spawnInkParticle(pos.getX()+0.5f, pos.getY()+1, pos.getZ()+0.5, 0, 0.15f, 0, color == -1 ? 0x99D9EA : ColorCollector.getColor(color));
				stack.shrink(1);
		...
		else if(worldIn.getBlockState(pos).getBlock() instanceof BlockCruxtruder)
		{
			...
			if(te instanceof TileEntityCruxtruder && worldIn.getBlockState(mainPos.up()).getBlock().isReplaceable(worldIn, mainPos.up()))
			{
				...
				worldIn.playSound(null, pos, FMPSounds.cruxtruderGelFill, SoundCategory.BLOCKS, 0.8f, 1);
				if(worldIn.isRemote) for(int i = 0; i < 16; ++i)
					FMPParticles.spawnInkParticle(mainPos.getX() + 0.5f, mainPos.getY() + 1, mainPos.getZ() + 0.5, 0, 0.15f, 0, color == -1 ? 0x99D9EA : ColorCollector.getColor(color));
