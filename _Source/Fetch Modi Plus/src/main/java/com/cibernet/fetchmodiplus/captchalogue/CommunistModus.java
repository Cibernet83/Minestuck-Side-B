...
import com.cibernet.fetchmodiplus.client.gui.captchalogue.CommunistGuiHandler;
...
public class CommunistModus extends Modus
{
	...
	public void readFromNBT(NBTTagCompound nbtTagCompound)
	{
		...
		if (this.side.isClient()) {
			...
			this.changed = true;
			FMPChannelHandler.sendToServer(FMPPacket.makePacket(FMPPacket.Type.REQUEST_COM_UPDATE));
	...
	public boolean sendUpdate()
	{
		if(!player.world.isRemote)
			...
			FMPChannelHandler.sendToPlayer(FMPPacket.makePacket(FMPPacket.Type.COM_UPDATE, writeToNBTGlobal(new NBTTagCompound())), p);
		return true;
	}
	...
	{
		if(!player.world.isRemote)
			FMPChannelHandler.sendToPlayer(FMPPacket.makePacket(FMPPacket.Type.COM_UPDATE, writeToNBTGlobal(new NBTTagCompound())), player);
		return true;
	}
