package com.cibernet.minestuckgodtier.commands;
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException
	{
		...
		switch (args[0])
		{
			...
			case "mastercontrol":
				...
				target.getCapability(MSGTCapabilities.GOD_TIER_DATA, null).setMasterControl(enable);
				target.getCapability(MSGTCapabilities.GOD_TIER_DATA, null).update();
			...
			case "maxbadges":
				...
				try
				{
					...
					target = (args.length >= 3) ? getPlayer(server, sender, args[2]) : getCommandSenderAsPlayer(sender);
					target.getCapability(MSGTCapabilities.GOD_TIER_DATA, null).setMaxBadges(count);
					target.getCapability(MSGTCapabilities.GOD_TIER_DATA, null).update();
			...
			case "lunarsway":
				...
				target = (args.length >= 3) ? getPlayer(server, sender, args[2]) : getCommandSenderAsPlayer(sender);
				target.getCapability(MSGTCapabilities.GOD_TIER_DATA, null).setLunarSway(sway);
				target.getCapability(MSGTCapabilities.GOD_TIER_DATA, null).update();
