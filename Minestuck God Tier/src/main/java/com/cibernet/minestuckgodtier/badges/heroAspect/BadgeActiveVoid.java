...
public class BadgeActiveVoid extends BadgeHeroAspect
{
    ...
    public void onBadgeUnlocked(World world, EntityPlayer player) {
        ...
        player.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).setVoidstepping(true);
    }
    ...
    public boolean onBadgeTick(World world, EntityPlayer player, IBadgeEffects badgeEffects, GodKeyStates.KeyState state, int time)
    {
        ...
        if(!player.isPotionActive(MSGTPotions.VOID_CONCEAL))
            MSGTChannelHandler.sendToTrackingAndSelf(MSGTPacket.makePacket(MSGTPacket.Type.SEND_PARTICLE, MSGTParticles.ParticleType.AURA, (player.ticksExisted % 2) == 0 ? 0x104EA2 : 0x001856, 1, player), player);
    ...
    public static void onLivingUpdate(LivingEvent.LivingUpdateEvent event)
    {
        ...
        player.noClip = player.noClip || (player.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).isVoidstepping() &&
                                                  (player.capabilities.isFlying || player.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).isDoingWimdyThing()));
    }
    ...
    public static void onPlayerCollision(GetCollisionBoxesEvent event)
    {
        ...
        if(event.getEntity().getCapability(MSGTCapabilities.BADGE_EFFECTS, null).isVoidstepping()
                   && (((EntityPlayer) event.getEntity()).capabilities.isFlying || event.getEntity().getCapability(MSGTCapabilities.BADGE_EFFECTS, null).isDoingWimdyThing()))
    ...
    {
        if(event.getEntity() != null && event.getEntityPlayer().isUser() && event.getEntity().getCapability(MSGTCapabilities.BADGE_EFFECTS, null).isVoidstepping())
    ...
    {
        if(Minecraft.getMinecraft().player != null && Minecraft.getMinecraft().player.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).isVoidstepping())
            event.setCanceled(true);
    }
}
