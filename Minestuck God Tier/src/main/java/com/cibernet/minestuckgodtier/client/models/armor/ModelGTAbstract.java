package com.cibernet.minestuckgodtier.client.models.armor;
...
public abstract class ModelGTAbstract extends ModelBiped
{
	...
	private void renderBody(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale)
	{
		...
		for(int i = 0; i < colorSet.length; i++)
		{
			...
			ResourceLocation loc = new ResourceLocation(MinestuckGodTier.MODID, "textures/models/armor/gt_"+heroClass.toString()+"_layer_"+(i+1)+".png");
		...
		Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation(MinestuckGodTier.MODID, "textures/models/armor/symbol/"+aspectName+".png"));
