...
public class GuiManageBadges extends GuiScreen
{
	...
	public static final ResourceLocation CURSOR = new ResourceLocation(MinestuckGodTier.MODID, "textures/items/sash_kit.png");
	...
	protected void setupBadges()
	{
		...
		{
			if(Minecraft.getMinecraft().player.getCapability(MSGTCapabilities.GOD_TIER_DATA, null).hasBadge(entry.getValue()))
			{
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks)
	{
		...
		//Sash
		mc.getTextureManager().bindTexture(new ResourceLocation(MinestuckGodTier.MODID, "textures/gui/sash_"+data.getLunarSway().toString().toLowerCase()+".png"));
		float n = width/xSize;
		for(int i = 0; i <= n; i++)
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
	{
		...
		{
			if((data.isBadgeActive(MSGTBadges.BADGE_OVERLORD) || data.hasBadge(hoveredBadge) && data.getMasterBadge() != null))
				MSGTChannelHandler.sendToServer(MSGTPacket.makePacket(MSGTPacket.Type.TOGGLE_BADGE, hoveredBadge));
		}
		else
		{
