...
public class PotionDecay extends MSGTPotionBase
{
	public static final DamageSource DECAY = new CritDamageSource("decay").setCrit().setDamageBypassesArmor();
	private static final ResourceLocation ICONS = new ResourceLocation(MinestuckGodTier.MODID, "textures/gui/icons.png");
	...
	{
		int decayTime = entityLivingBaseIn.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).getDecayTime();
		entityLivingBaseIn.attackEntityFrom(DECAY, (decayTime++)/2);
		if (!entityLivingBaseIn.world.isRemote)
			entityLivingBaseIn.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).setDecayTime(decayTime);
	}
	...
	public void removeAttributesModifiersFromEntity(EntityLivingBase entityLivingBaseIn, AbstractAttributeMap attributeMapIn, int amplifier)
	{
		...
		entityLivingBaseIn.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).setDecayTime(0);
	}
	...
	{
		if (!(event.getType() == RenderGameOverlayEvent.ElementType.HEALTH && Minecraft.getMinecraft().player.isPotionActive(MSGTPotions.DECAY)))
			return;
