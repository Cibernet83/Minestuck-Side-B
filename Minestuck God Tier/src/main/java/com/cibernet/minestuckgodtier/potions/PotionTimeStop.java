...
public class PotionTimeStop extends PotionMouseSensitivityAdjusterBase
{
	...
	public static void onLivingUpdate(LivingEvent.LivingUpdateEvent event)
	{
		...
		if (entity.world.isRemote ?
				entity.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).isTimeStopped() :
				entity.isPotionActive(MSGTPotions.TIME_STOP)) // Usually I would use capabilities, but we need this instead to update it
		{
			...
			if (entity.world.isRemote)
			{
				int color = MSGTPotions.TIME_STOP.getLiquidColor();
			...
			if (!entity.world.isRemote || (MSGTUtils.isClientPlayer(entity) && entity.isPotionActive(MSGTPotions.TIME_STOP)))
			{
				PotionEffect potionEffect = entity.getActivePotionEffect(MSGTPotions.TIME_STOP);
				if (!potionEffect.onUpdate(entity) && !entity.world.isRemote)
					if (!net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new net.minecraftforge.event.entity.living.PotionEvent.PotionExpiryEvent(entity, potionEffect)))
						entity.removePotionEffect(MSGTPotions.TIME_STOP);
			}
	...
	public static void onGuiOpen(GuiOpenEvent event)
	{
		...
		if (Minecraft.getMinecraft().player != null && Minecraft.getMinecraft().player.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).isTimeStopped())
		{
	...
	public void applyAttributesModifiersToEntity(EntityLivingBase entityLivingBaseIn, AbstractAttributeMap attributeMapIn, int amplifier)
	{
		...
		entityLivingBaseIn.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).setTimeStopped(true);
	}
	...
	public void removeAttributesModifiersFromEntity(EntityLivingBase entityLivingBaseIn, AbstractAttributeMap attributeMapIn, int amplifier)
	{
		...
		entityLivingBaseIn.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).setTimeStopped(false);
	}
}
