...
public class PacketMindflayerMovementInput extends MSGTPacket
{
	...
	{
		EntityLivingBase target = player.getCapability(MSGTCapabilities.BADGE_EFFECTS, null).getMindflayerEntity();
		...
		{
			IBadgeEffects badgeEffects = target.getCapability(MSGTCapabilities.BADGE_EFFECTS, null);
			...
			if(target instanceof EntityPlayer && ((EntityPlayer) target).inventory.currentItem != currentItem)
			{
				...
				MSGTChannelHandler.sendToPlayer(makePacket(Type.SET_CURRENT_ITEM, currentItem), (EntityPlayer) target);
			}
		}
	}
