...
import com.cibernet.minestuckgodtier.damage.IMSGTDamage;
...
public class KarmaEventHandler
{
	...
	public static void onPlayerDeath(LivingDeathEvent event)
	{
		if(event.getEntityLiving() instanceof EntityPlayer)
		{
			...
			IGodTierData targetData = event.getEntityLiving().getCapability(MSGTCapabilities.GOD_TIER_DATA, null);
			...
			if(pvpKill)
			{
				...
				IGodTierData sourceData = event.getSource().getTrueSource().getCapability(MSGTCapabilities.GOD_TIER_DATA, null);
			...
			{
				int minKarma = targetData.isBadgeActive(MSGTBadges.KARMA) ? 40 : 20;
				...
				} else
				{
					...
					boolean hasRevenantBadge = targetData.isBadgeActive(MSGTBadges.REVENANTS_RETALIATION);

					player.addPotionEffect(new PotionEffect(MSGTPotions.GOD_TIER_COMEBACK, hasRevenantBadge ? 500 : 200, targetData.isBadgeActive(MSGTBadges.EFFECT_BUFF) ? 2 : 0));
	...
	public static void onPlayerDamage(LivingDamageEvent event)
	{
		if(event.getEntityLiving() instanceof EntityPlayer && event.getSource().getTrueSource() instanceof EntityPlayer)
		{
			...
			{
				int targetKarma = event.getEntityLiving().getCapability(MSGTCapabilities.GOD_TIER_DATA, null).getTotalKarma();
				IGodTierData sourceData = event.getSource().getTrueSource().getCapability(MSGTCapabilities.GOD_TIER_DATA, null);
	...
	public static void onPlayerTick(TickEvent.PlayerTickEvent event)
	{
		if(event.phase != TickEvent.Phase.START)
			return;
		IGodTierData data = event.player.getCapability(MSGTCapabilities.GOD_TIER_DATA, null);
		if(data.getTempKarma() != 0)
		{
			data.setTempKarma(data.getTempKarma() - (1f / 2400f * (data.isBadgeActive(MSGTBadges.KARMA) ? 2 : 1)) * Math.signum(data.getTempKarma()));
